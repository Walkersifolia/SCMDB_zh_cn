...
|---|---|---|---|---|---|
| **4.10.0-live.12572603**（当前生效） | 6349 | 6144 | 0 | 1720 | 69 |
| 4.10.0-ptu.12409360（PTU 备用） | 4303 | 4175 | 0 | 60 | 68 |

说明：
- 翻译范围仅限游戏内数据（任务、地点、船只、物品、阵营等）；SCMDB 网站 UI 本身不参与本地化。
- 「已翻译」统计口径为 `tr != en`（实际显示中文的条目数）；未翻译 205 条 = 任务标题/描述（官方值为 `~mission(...)` 引用式，82 条）+ 位置 ID（58）+ 制造槽位（64）+ 矿元素兜底（1），均为无对应中文文本的英文兜底，属正常。
