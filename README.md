...
|---|---|---|---|---|---|
| **4.10.1-live.12660092**（当前生效） | 6349 | 6144 | 0 | 1720 | 69 |
| 4.10.0-ptu.12409360（PTU 备用） | 4303 | 4175 | 0 | 60 | 68 |

说明：
- 翻译范围仅限游戏内数据（任务、地点、船只、物品、阵营等）；SCMDB 网站 UI 本身不参与本地化。
- 「已翻译」统计口径为 `tr != en`（实际显示中文的条目数）；未翻译 205 条 = 任务标题/描述（69 条，部分官方值为 `~mission(...)` 引用式）+ 位置 ID（60）+ 制造槽位（64）+ 矿元素兜底（1）+ 其他无中文标签（11 条：货梯 SCU、占位符哨兵、UI 数值），均为无对应中文文本的英文兜底，属正常。
